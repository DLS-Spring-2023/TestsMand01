...
export const generateCpr = (dob: Date = generateDob(), gender?: Gender): string => {
	const day = String(dob.getDate()).padStart(2, '0');
	const month = String(dob.getMonth() + 1).padStart(2, '0');
	const year = String(dob.getFullYear()).slice(2);

	return `${day}${month}${year}${generateRandomDigits(3)}${
		gender === Gender.Male ? getRandomOddNumber() : getRandomEvenNumber()
	}`;
};
